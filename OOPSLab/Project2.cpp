...
string name_save[10][3]; // global variable where all names are saved

class Node{ // can be treated as a room
public:
    int room_capacity = 1;     //toal vacancy left
    int fill_room = 0;         // total people who occupied the room 
    char name[3][10];
    Node *next;
    Node *previous;
};
...
class hostel{
    Node *head[3];        //points to first room of each floor
    Node *created_node;
public:
    hostel()
    {
...
void hostel :: display(){
    ...
    while (created_node != NULL){
        if (created_node->room_capacity != 0){
            j++;
            cout << " Room no: " << j;
        ...
        if (secondfloor_node->room_capacity != 0){
            k++;
            cout << "\t Room no: " << k;
        ...
        if( thirdfloor_node->room_capacity != 0){
            l++;
            cout << "\t Room no: " << l;
