...
checkd(graph,v);
    ...
    static void checkd(ArrayList<ArrayList<Edge>> graph,int v)
    {
        boolean vis[]=new boolean[v];
for(int i=0;i<v;i++)
{
    if(vis[i]==false){
 boolean ans=Ddfs(graph,i,vis,-1);
 if(ans==true)
 {
...
return;
   ...
   static boolean Ddfs(ArrayList<ArrayList<Edge>> graph ,int cu,boolean vis[],int p)
   {
vis[cu]=true;
for(int i=0;i<graph.get(cu).size();i++)
{
    ...
    if(vis[e.dest]==false){
    boolean ans=Ddfs(graph,e.dest,vis,cu);
   if(ans==true)
   return true;
}
    else if(p!=e.dest){
